...
Чтобы работать как оператор, администратор должен:

1. Добавить вас в список операторов (имя + ваш Telegram ID).
2. Привязать ваш Telegram ID к записи оператора.

Без привязки Telegram ID панель оператора **не откроется**.

### 1.2. Открыть панель

| Действие | Как |
|----------|-----|
| Первый вход | `/start` — появится кнопка Mini App и **панель оператора** |
| Вернуться в панель | `/panel` или `/operator` |
| Отменить текущее действие | `/cancel` |
| Справка | `/help` или кнопка **ℹ️ Помощь** |

> Команды работают и в виде `/panel@имя_бота`.
...
**Правило:** кто внёс клиента в систему — тот его и ведёт.  
...
| **➕ Клиент** | Добавить нового клиента по телефону |
...
3. Бот выдаст **ID** (например `12`) и откроет карточку

Клиент сразу закрепляется за вами как оператором.
...
```
➕ Клиент → телефон → ✏️ Данные (имя) → 🏷 Теги → «В работе» → Без вложений
```
...
```mermaid
flowchart TD
    A[Начало смены /panel] --> B[📅 Сегодня — сверка]
    B --> C{Новый клиент?}
...
Так и задумано — вы видите только своих.
