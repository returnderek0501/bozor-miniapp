...
| Действие | Как |
|----------|-----|
| Первый вход | `/start`, затем отправьте секретный номер, выданный администратором |
| Вернуться в панель | Снова отправьте секретный номер; в активной сессии также работают `/panel` и `/operator` |
| Отменить текущее действие | `/cancel` |
| Справка | `/help` или кнопка **ℹ️ Помощь** |

> Меню откроется только при совпадении секретного номера **и** Telegram ID из списка операторов/администраторов. Служебная сессия действует ограниченное время.
