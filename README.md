...
**Служебный веб-интерфейс:** оператор или администратор открывает Mini App через кнопку меню Telegram и вводит номер `887766`. Сервер дополнительно проверяет Telegram ID по сохранённым спискам ролей. Веб-сессия действует 12 часов.

**Браузерная админ-панель:** администраторы могут открыть секретный путь (по умолчанию `https://<host>/ops-uztronix-x7m2`), войти по своему Telegram ID и номеру `887766`. Это тот же интерфейс, но в обычном браузере с большим окном. Путь можно сменить через `BROWSER_ADMIN_PATH`. На этот же URL можно повесить отдельный домен в Railway.
